...
Corpus-first generator of **Austrian-curriculum-anchored teaching material** for AHS secondary schools.
A teacher inputs topic + grade + time → a ready-to-use, competence-anchored bundle — **assembled
deterministically from a curated, human-reviewed corpus** — correct by construction, provably
competence-aligned, and portable into the tools they already use. Generation (LLM/subagent campaigns)
happens **offline in the corpus loop**, upstream of a human review gate; the delivery loop is LLM-free
(the offline-first pivot, 2 Jul 2026 — see `Documents/invariants.md` §10).
...
python -m pytest -q         # 369 tests, all offline (no API key needed)
...
- **Generation is corpus-loop work** (offline-first pivot — `project-handoff.md` §4): subagent campaigns
  need no key; with `ANTHROPIC_API_KEY` set (`claude-opus-4-8`, adaptive thinking) the API seam generates
  too — in both cases into the review gates, never straight to a user. **Without a key**, the
  master-library subjects fall back to their hand-authored content objects so the whole loop is demoable
  offline.
