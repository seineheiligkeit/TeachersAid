...
| Verify | `pipeline/verify.py` | rules (kinds/dimensions/coverage/depth/difficulty/**media-policy**/**chart-sanity**/**(c)-data-label**/**numeric-claims**); LLM fact-check optional. The **(c)-label gate** (`pipeline/figure_lint.py`) warns when a data figure carries real-looking numbers but declares neither `data_source` (sourced+cited) nor `illustrative` (schematic) — the grounded-facts honesty rule. The **numeric-claims lint** (`pipeline/number_lint.py`, Track 1 #2) warns when a number in a task's prose (prompt/answer_key/acceptable_reasoning) next to a SOURCED figure is not **derivable from the cited series** — closure = values · contiguous-range sums · cross-series sums/same-range diffs · means/medians · shares & within-array ratios in %, matched at the precision the prose claims ("ca. 129.000" matches 129 086); German formats (dot/space thousands, comma decimals, Mio./Mrd.), dates stripped, bare numbers <120 and unformatted years skipped (calibrated on the GWB data pass — advisory lane, the SME judges). Closes the authored-number hole from the 2 Jul review and doubles as the **anti-rot check** (a stale baked number surfaces on dataset refresh). |
...
### The figure styleguide + the scene engine (Session 10) — read `Documents/figure-styleguide.md`

Two additions make figures **consistent** and **composable**. Full design doc:
`Documents/figure-styleguide.md`.

- **`pipeline/figstyle.py` — the styleguide.** One source of truth for how a figure looks:
  **semantic colour ROLES** (`ink`/`primary`/`focus`/`positive`/`negative`/`muted`/`surface` — colour
  MEANS something: `focus` is *always* the unknown/result/region of interest), a **categorical ramp**
  (`CATEGORICAL`, so multi-series stops silently using matplotlib defaults) and a **dash ramp**
...
- **`pipeline/scene.py` — the scene engine.** A figure as data: a `Scene` = `Canvas` + ordered typed
  ...
  the SAME scene renders at different **stages/densities** (a step-by-step construction worksheet from one
  computed object). Renderer-agnostic in shape. **Two-tier, like `choose_representation`: the LLM never
  authors a Scene** (that would let it draw a wrong tangent / leak an answer); a **didactic recipe COMPUTES
  it** from a small correct-by-construction spec.
  - **`pipeline/constructions.py`** — `matplotlib:triangle_construction`: the *merkwürdige Punkte des
    Dreiecks* (Umkreis/Inkreis/Schwerpunkt/Höhenschnittpunkt/Eulergerade/Feuerbachkreis), all computed from
    3 vertices; `stage` 1–6 for the step-by-step. Move a vertex → everything re-solves.
  - **`pipeline/calculus.py`** — the analysis family via **sympy** (`tangent_slope`/`definite_integral` are
...
## Delivery loop + campaign planner (offline-first Tracks 1–2, built 2 Jul 2026)

The first build slice of the pivot's program (`feature-roadmap.md` "Start here"):
...
- **`pipeline/deliver.py::deliver`** — the LLM-free delivery read-path (invariants §10): serve a
  **vetted library worksheet** (deterministic match: KB-competence overlap ×3 + topic-term overlap +
  ...
  `DemandRecord`. `POST /api/deliver`; a test form lives in the Statistik tab. Inter-block coherence
  (Track 2's hard problem) is deliberately NOT here yet — this is the retrieval hierarchy + gap outlet.
- **`store/demandstore.py::DemandStore`** (a `JsonStore`, `runs/demand/`) — the **Wunschliste**: a gap
  becomes an `open` wish (subject·Klasse·topic/KB·envelope + the honest reason), lifecycle
  open→planned→fulfilled/dismissed (`GET /api/demand`, `POST /api/demand/{id}/status`). Demand feeds
  the next campaign — the corpus loop's intake.
- **`stats.coverage_map` / `campaign_gaps`** — the **campaign planner**: one cell per (Stufe·Fach·
  Klasse·Kompetenzbereich), approved task blocks split by Anforderungsband, scope variety, status
  **gruen** (≥`GREEN_MIN_TASKS`=4 tasks AND all 3 Bänder — the floor compose needs to span
  leicht→anspruchsvoll) / **teil** / **leer**. `GET /api/coverage` (light payload);
  `GET /api/coverage/gaps` exports the non-green cells WITH verbatim competence ids — the
  campaign-brief anchors `tools/breadth_prompt.py` consumes. The Statistik tab renders per-subject
  progress bars + expandable cell tables — **the corpus progress bar** (at build time: 610 cells,
  95 grün = 16 %; GWB Unterstufe 14/14 grün). Locked by `tests/test_{number_lint,coverage,deliver}.py`.
