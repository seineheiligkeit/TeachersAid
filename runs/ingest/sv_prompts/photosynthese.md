# Sachverhalt-Autorierung: „Die Photosynthese" (Biologie und Umweltbildung, 3. Klasse)
...
**Leitfrage (Vorschlag):** Wie macht eine Pflanze aus Licht, Wasser und Luft ihre Nahrung?

**Kurator-Hinweis:** Linearer Prozess (Licht → Wasser+CO₂ → Traubenzucker+Sauerstoff). Begriffe: Chlorophyll, Spaltöffnungen, Glucose. Kein Datum — die Schrittfolge IST die Ordnung.
...
## Kompetenzen (Biologie und Umweltbildung, 3. Klasse — verbatim; `competences` MUSS aus diesen IDs wählen)
...
## Ausgabe
Schreibe **genau eine** Datei nach `runs/ingest/sv/photosynthese.json` — **ausschließlich** dieses JSON
...
```json
{
  ...
  "topic": "Die Photosynthese",
  "leitfrage": "Wie macht eine Pflanze aus Licht, Wasser und Luft ihre Nahrung?",
  "competences": [
    "<ID aus der Liste>",
    "<weitere ID>"
  ],
