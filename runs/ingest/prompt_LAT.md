...
## Sprache (WICHTIG)
Das **Sprachmaterial** (Texte, Dialoge, Wortschatz, Beispielsätze, die die Schüler:innen bearbeiten) ist in **Latein**. Arbeitsanweisungen dürfen Deutsch oder Latein sein (Unterstufe: oft Deutsch als Gerüst, später mehr Latein). Die **Lehrkraft-Ebene** (throughline/talking_points/extensions) und `watch_outs` bleiben **Deutsch**. `answer_key` in Latein (Modelllösung), bei Bedarf mit kurzer deutscher Notiz.
