# KAMPAGNEN-AUFTRAG: Physik, Klasse 2 — 2 Arbeitsblätter, EXAKT eines je Kompetenzbereich

Dieses Set schließt gezielt Korpus-Lücken. Die Blätter:
1. Blatt 1: KB **"Optische Systeme"** — noch leer: volle Bandbreite nötig
2. Blatt 2: KB **"Sehen und Hören"** — noch leer: volle Bandbreite nötig
...
Setze im JSON `"kompetenzbereich"` exakt auf den KB-Namen des Blatts und `"klasse": 2`. Wähle je Blatt eine eigene, kreative Kernfrage (kein KB-Namens-Echo).
...
## Ausgabe
Schreibe **2 Dateien**: `runs/ingest/campaign_mint_us/PHY_kl2_1.json` … `runs/ingest/campaign_mint_us/PHY_kl2_2.json`.
Jede Datei ist **ausschließlich** dieses JSON (kein Fließtext, keine ``` Zäune):

```json
{
  "subject": "Physik", "klasse": 2,
