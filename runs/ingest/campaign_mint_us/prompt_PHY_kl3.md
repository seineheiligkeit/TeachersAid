# KAMPAGNEN-AUFTRAG: Physik, Klasse 3 — 1 Arbeitsblätter, EXAKT eines je Kompetenzbereich

Dieses Set schließt gezielt Korpus-Lücken. Die Blätter:
1. Blatt 1: KB **"Energie"** — noch leer: volle Bandbreite nötig
...
Setze im JSON `"kompetenzbereich"` exakt auf den KB-Namen des Blatts und `"klasse": 3`. Wähle je Blatt eine eigene, kreative Kernfrage (kein KB-Namens-Echo).
...
## Fach-Hinweis
Abbildungen tragen Physik: `function_graph` für Weg-Zeit-/v-t-Verläufe, `number_line` für Skalen (Frequenz, Schallpegel, Brennweite), `line` für Messreihen. Alltagsnahe Kontexte (Brille, Fotoapparat, Musikinstrument, E-Bike) statt Formel-Drill.
...
- Beschreibung von (auch zweidimensionalen) Bewegungen, Tempo und Geschwindigkeit · Je-desto-Zusammenhang zwischen der Änderung einer Geschwindigkeit und einer Einwirkung von außen (qualitativer Zugang zur newtonschen Bewegungsgleichung in der Form · Phänomenologische Behandlung von Kraftarten · Wechselwirkungsgesetz · Permanent- und Elektromagnetismus · Einfacher Stromkreis: Stromstärke, Spannung, Widerstand, Wirkungen des elektrischen Stroms, Gefahren und Schutzmaßnahmen · Modellvorstellungen (zB Teilchenmodelle, Eisen-Magnet-Modell für den Magnetismus, Elektronengasmodell für den Stromkreis) · Elektrische und mechanische Energie, Energieerhaltung · Elektromotor- und Generatorprinzip · Erste Einblicke in die Funktionsweise moderner elektronischer Geräte
...
- `geosphere_klima_normal_1991_2020` — Klimanormalwerte österreichischer Stationen (1991–2020). Serien: `wien` (Wien (Hohe Warte)) · `innsbruck` (Innsbruck (Universität)) · `bregenz` (Bregenz) · `graz` (Graz (Universität)). Datenquelle: GeoSphere Austria – data.hub.geosphere.at (klima-v2-1m, CC BY 4.0)
...
## Ausgabe
Schreibe **1 Dateien**: `runs/ingest/campaign_mint_us/PHY_kl3_1.json` … `runs/ingest/campaign_mint_us/PHY_kl3_1.json`.
Jede Datei ist **ausschließlich** dieses JSON (kein Fließtext, keine ``` Zäune):

```json
{
  "subject": "Physik", "klasse": 3,
