# Realie-Autorierung: „a weekly weather forecast" (English, Erste lebende Fremdsprache, 3. Klasse, A2)

Du schreibst **eine** `Realie` — ein **erfundenes, alltagsnahes** Lesedokument in **English**
(a weekly weather forecast), das als **Sprechanlass** dient. Eine Realie ist **kein** Sachtext: die „Fakten" (Zeiten,
...
**Szene / Kommunikationsanlass:** Das Wetter — read the forecast and decide what to wear / what to do.

**Kurator-Hinweis:** A 5-day forecast: day · weather · temperature (e.g. 18°C). Temperatures are not lint-checked (no £/time) — the value is the reasoning + speaking.
...
## Ausgabe
Schreibe **genau eine** Datei nach `runs/ingest/realien/weather.json` — **ausschließlich** dieses JSON
(kein Fließtext, keine ``` Zäune), genau in dieser Form:

```json
{
  "id": "fs-realie-weather",
  "title": "<prägnanter L2-Titel>",
  "subject": "Erste lebende Fremdsprache",
  "klasse": 3,
  "genre": "a weekly weather forecast",
  "textsorte": "Realie",
  "cefr": "A2",
  "origin": "constructed",
  "scene": "Das Wetter",
